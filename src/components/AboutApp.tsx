          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-5 bg-slate-50 rounded-xl border border-slate-100 flex gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 leading-relaxed">
                  Перевірка знань за допомогою інтерактивних тестів. Підтримуються різні формати тестування, розбір робочих кейсів та розширена база зі 100 автентичних висловлювань видатних українських діячів (Шевченка, Франка, Сковороди, Лесі Українки, Стуса, Костенко та інших) для мотивації та підтримки під час відповідей. Показ цих висловлювань у тестах адміністратор може вимкнути в розділі «Адміністрування → Налаштування».
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Тривалість проходження.</strong> Система фіксує, скільки часу тривала кожна спроба — від натискання «Розпочати тестування» до завершення (кнопкою або автоматично, коли вичерпано ліміт часу). Час видно одразу на екрані результату, у колонці «Тривалість» історії тестувань (разом із середнім часом усіх спроб), у журналі дій та у звіті «Результати тестування» аналітики — як середній час загалом і для кожного діапазону балів, зокрема у вивантаженні в Excel. Для спроб, пройдених до появи цієї можливості, тривалість не відома й показується прочерком.
                </p>
              </div>
            </div>
            ...
            <div className="p-5 bg-slate-50 rounded-xl border border-slate-100 flex gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Хто бачить кейси:</strong> кожен кейс прив'язаний до інструкції, тож співробітник бачить кейси тих інструкцій, до яких має доступ, а також загальні кейси та кейси, призначені кроками його онбордингу. У курсі з увімкненими «Практичними кейсами» після завершення з'являється кнопка з кейсами всіх інструкцій курсу. Над кейсом видно, до якої інструкції й підрозділу він належить.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Кейс за допомогою ШІ:</strong> у формі створення кейсу спершу оберіть інструкцію, а потім натисніть «Згенерувати». ШІ складе робочу ситуацію та 3–4 варіанти дій (рівно один правильний) з поясненням до кожного, спираючись лише на текст цієї інструкції. Можна додати побажання — наприклад, «конфліктний клієнт» або «повернення товару», — а ШІ не повторюватиме вже наявні кейси до цієї інструкції. Результат лише підставляється у поля форми: перевірте й за потреби виправте його, перш ніж натиснути «Створити».
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Результати:</strong> коли співробітник доходить до кінця серії кейсів, результат (скільки ситуацій розв'язано правильно з першого разу і скільки часу це зайняло) зберігається в історії з позначкою «Кейси». Це тренування, а не атестація: кейси не впливають на найкращий бал тесту, на можливість підписати ознайомлення, на сертифікати чи ліміт спроб тесту. Якщо кейс відкрито з каталогу, після останнього кейсу ви повертаєтесь до каталогу.
                </p>
              </div>
            </div>
            ...
            <div className="p-5 bg-slate-50 rounded-xl border border-slate-100 flex gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 leading-relaxed">
                  ...
                  {' '}Якщо захисні фільтри основної моделі ШІ помилково відхилять документ, запит автоматично виконає запасна модель, тож ви отримаєте результат замість помилки. Це стосується всієї ШІ-генерації: розбору документів, догенерації питань і створення кейсів. Повідомлення про відмову з'явиться, лише якщо відмовлять обидві моделі.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
            ...
            <div className="p-5 bg-slate-50 rounded-xl border border-slate-100 flex gap-4">
              ...
              <div>
                <h3 className="font-bold text-slate-900 mb-1">Єдине меню «Дії» для кожного матеріалу</h3>
                <p className="text-sm text-slate-600 leading-relaxed">
                  У всіх переліках групи «Матеріали» в «Адмініструванні» — інструкції, курси, кейси, простори та життєвий цикл «Бази знань», онбординги й призначення — рядок матеріалу тепер влаштований однаково: на виду лише кнопка <strong>«Перегляд»</strong>, а все інше керування зібране в кнопці <strong>«Дії»</strong>. Так рядки не переповнюються кнопками, скільки б нових можливостей не з'являлося, і кожна дія має зрозумілу назву з коротким поясненням замість незрозумілої іконки.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  У меню дії розкладені за групами: наприклад, для інструкції — «Редагування» (налаштування, вміст, питання тесту з їхньою кількістю) та «Експорт» (Markdown чи PDF); для матеріалу в «Базі знань» — «Життєвий цикл» (статус, нова версія, історія ревізій). Небезпечні дії — видалення, архівація, скасування призначення — завжди стоять унизу червоним і перепитують просто в меню, перш ніж щось зробити. Недоступна дія показується сірою з поясненням, чому саме (наприклад, «Спершу опублікуйте онбординг»). Меню працює і з клавіатури: стрілки вгору/вниз, Enter, Esc. В онбордингах «Перегляд» відкриває схему маршруту.
                </p>
              </div>
            </div>
            ...
            <div className="p-5 bg-slate-50 rounded-xl border border-slate-100 flex gap-4">
              ...
              <div>
                <h3 className="font-bold text-slate-900 mb-1">Налаштування додатка</h3>
                <p className="text-sm text-slate-600 leading-relaxed">
                  В «Адмініструванні» з'явився розділ <strong>«Налаштування»</strong> з глобальними налаштуваннями, що діють одразу для всіх користувачів. У бічному меню адміністрування він разом із «Дашбордами», «Журналом адміністратора» та «Журналом дій» винесений в окрему групу <strong>«Адміністрування»</strong>, під групами «Матеріали» та «Організація». Кожен пункт групи видно лише тим, кому він доступний. Кожне налаштування має перемикач, а зміна зберігається одразу, без кнопки «Зберегти».
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Висловлювання українських діячів у тестах:</strong> вмикає або вимикає мотиваційні цитати, що з'являються після відповіді під час тесту, і підсумкову цитату на екрані результату. «Цитата дня» на головній сторінці від цього не залежить. За замовчуванням висловлювання ввімкнені.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  Розділ бачать адміністратори, а також ролі, яким у «Ролях та правах» надано право «Керування налаштуваннями додатка».
                </p>
              </div>
            </div>
            ...
            <div className="p-5 bg-slate-50 rounded-xl border border-slate-100 flex gap-4">
              ...
              <div>
                <h3 className="font-bold text-slate-900 mb-1">Дашборди</h3>
                <p className="text-sm text-slate-600 leading-relaxed">
                  У меню «Адміністрування» → група «Адміністрування» з'явився розділ <strong>«Дашборди»</strong> — одне місце, де зібрані зведені показники. Зараз у ньому два дашборди:
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>«Аналітика та прогрес навчання»</strong> — той самий екран, що відкривається через «Мій профіль» → «Профіль»: успішність, історія тестів, опрацьовані матеріали й сертифікати, власні або будь-якого співробітника.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>«Активність»</strong> — як співробітники користуються порталом за обраний період (7, 30, 90 днів або власні дати): кількість входів, кількість активних користувачів, загальний час у додатку й середній час на людину. Біля головних показників видно, наскільки вони зросли чи знизилися порівняно з попереднім таким самим періодом. Також показано охоплення (скільки співробітників узагалі заходили, а скільки — ні), графіки входів і активних користувачів по днях, час у додатку по днях, у які години й дні тижня люди заходять найчастіше, найпопулярніші розділи й матеріали, перелік найактивніших співробітників і охоплення за підрозділами.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Деталізація: хто саме пораховано.</strong> На обох дашбордах майже кожен блок можна натиснути, щоб побачити, з чого складається число. У дашборді «Активність» це всі картки й показники (входи, активні користувачі, час, охоплення, «не заходили», перегляди матеріалів, тести, підписи ознайомлень, невдалі входи, автовиходи), будь-який день на графіках, година чи день тижня, рядок у «Популярних розділах» і «Популярних матеріалах», а також підрозділ і людина в таблицях. Відкривається перелік із пошуком і вкладками («Заходили / Не заходили», «Співробітники / Невідомі логіни» тощо). З переліку можна перейти глибше: натиснути на день — і побачити, хто був у порталі цього дня, натиснути на людину — і побачити її дні з кількістю входів і часом; стрілка «Назад» повертає до попереднього переліку. У дашборді «Аналітика та прогрес навчання» натискаються картки «Найкращий бал» (яка спроба дала цей бал), «Завершено тестів», «Прочитано інструкцій» (які прочитані, а які ще ні) і «Всього відповідей», стовпчик підрозділу на графіку середнього балу та кругова діаграма прогресу читання.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  Час у додатку — це активний час: він накопичується, поки людина працює з порталом (натискає, гортає, друкує), а перерви довші за 5 хвилин без жодної дії не зараховуються. Дві відкриті вкладки час не подвоюють. Облік часу ведеться від моменту появи дашборда, тож за попередні дні час не показується. Дашборд «Активність», як і «Журнал дій», доступний лише користувачам з роллю «Адміністратор».
                </p>
              </div>
            </div>
            ...
            <div className="p-5 bg-slate-50 rounded-xl border border-slate-100 flex gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Підтвердження пароля:</strong> коли ви задаєте пароль новому користувачеві або змінюєте пароль наявному, система просить ввести його двічі. Під другим полем одразу видно, чи паролі збігаються. Зберегти зміни можна, лише коли обидва введення однакові, тож одруківка не заблокує співробітнику вхід. Якщо поле нового пароля при редагуванні лишити порожнім, пароль не зміниться.
                </p>
              </div>
            </div>
            ...
            <div className="p-5 bg-slate-50 rounded-xl border border-slate-100 flex gap-4">
              ...
              <div>
                <h3 className="font-bold text-slate-900 mb-1">Ролі та оргструктура — такими ж таблицями</h3>
                <p className="text-sm text-slate-600 leading-relaxed">
                  Вкладки «Ролі та права» та «Організація» тепер виглядають так само, як «Користувачі»: таблиця з пошуком, фільтрами, сортуванням кліком по заголовку колонки та сторінками по 25, 50 або 100 рядків. Над таблицею видно, скільки записів знайдено.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Ролі та права:</strong> для кожної ролі видно назву й ключ, тип (системна чи користувацька), опис, кількість прав і кількість користувачів. Ролі можна відфільтрувати за типом або за конкретним правом — так легко знайти, хто, наприклад, може редагувати матеріали. Клік по рядку відкриває матрицю прав окремим вікном; там же можна видалити користувацьку роль.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Організація:</strong> підрозділи, посади й локації — окремі таблиці з лічильниками на перемикачах. Посади можна відфільтрувати за підрозділом, усі довідники — за статусом (активні / вимкнені). Кнопка «Додати…» та клік по рядку відкривають форму окремим вікном, де тепер також можна задати порядок підрозділу, підрозділ і грейд посади, країну й часовий пояс локації, а також вимкнути запис, не видаляючи його. Системний підрозділ «Всі підрозділи» як і раніше захищений від змін.
                </p>
              </div>
            </div>
            ...
            <div className="p-5 bg-slate-50 rounded-xl border border-slate-100 flex gap-4">
              ...
              <div>
                <h3 className="font-bold text-slate-900 mb-1">Відео YouTube в інструкціях</h3>
                <p className="text-sm text-slate-600 leading-relaxed">
                  В інструкцію можна додати відео з YouTube — воно показується прямо в тексті вбудованим плеєром з обкладинкою та підписом. Відео запускається лише після натискання кнопки відтворення, тож сторінка з кількома відео відкривається так само швидко. Під плеєром є посилання «Відкрити на YouTube».
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Як додати:</strong> у редакторі вмісту інструкції натисніть <strong>«Відео YouTube»</strong>, вставте посилання на відео та, за бажанням, підпис. Підходять будь-які звичні посилання: з адресного рядка браузера, з кнопки «Поділитися» (youtu.be), на Shorts чи трансляції; посилання з позначкою часу (наприклад, …?t=90) запускає відео з потрібної хвилини. Можна й просто написати посилання окремим рядком у тексті. Якщо посилання стоїть посеред речення, воно лишається звичайним посиланням зі значком відео і відкривається в новій вкладці.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  Під час ШІ-обробки документа посилання на відео YouTube з оригіналу зберігаються й одразу стають плеєрами в тексті інструкції.
                </p>
              </div>
            </div>
            ...
            <div className="p-5 bg-indigo-50/60 rounded-xl border border-indigo-200/80 flex gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
                  <strong>Швидкий пошук співробітника:</strong> при створенні призначення конкретній людині не потрібно гортати довгий список. Почніть вводити частину ПІБ, email або логіна, і перелік одразу звузиться до тих, хто підходить. Можна вводити кілька слів у будь-якому порядку, наприклад «петро іван». Регістр і різні апострофи не мають значення. Обрати людину можна мишею або стрілками й клавішею Enter, а хрестик у полі скидає вибір. Шукати можна серед усіх співробітників компанії, навіть якщо їх понад тисячу.
                </p>
                <p className="text-sm text-slate-600 leading-relaxed mt-2">
