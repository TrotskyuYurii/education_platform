  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3.5">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs">
          <div className="flex items-center justify-between text-emerald-600 text-xs font-medium mb-1.5">
            <span>Виконано вчасно</span>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs">
          <div className="flex items-center justify-between text-blue-600 text-xs font-medium mb-1.5">
            <span>В процесі</span>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-2xs">
          <div className="flex items-center justify-between text-amber-600 text-xs font-medium mb-1.5">
            <span>Очікують початку</span>
        ...
        <div className={`rounded-xl border p-4 shadow-2xs ${
          ...
          <div className="flex items-center justify-between text-rose-600 text-xs font-medium mb-1.5">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                <tr>
                  <th className="py-3 px-4">Співробітник</th>
                  <th className="py-3 px-4">Матеріал</th>
                ...
                {filteredAssignments.map(a => {
                  ...
                  return (
                    <tr key={a.id} className={`hover:bg-slate-50/70 transition ${isOverdue ? 'bg-rose-50/20' : ''}`}>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-500 flex items-center gap-1.5 mt-0.5">
                              <span>{a.user?.email || a.user?.username}</span>
                              {a.department && (
                                <>
                                  <span>•</span>
                      ...
                      <td className="py-3.5 px-4">
                        {isCompleted && (
                          <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold bg-emerald-100 text-emerald-800">
                            <CheckCircle2 className="w-3.5 h-3.5" />
                            <span>Завершено {a.score !== undefined ? `(${a.score}%)` : ''}</span>
                          </div>
                        )}
                        {!isCompleted && isOverdue && (
                          <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold bg-rose-100 text-rose-800">
                            <AlertTriangle className="w-3.5 h-3.5" />
                            <span>Прострочено</span>
                          </div>
                        )}
                        {!isCompleted && !isOverdue && a.status === 'in_progress' && (
                          <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-blue-100 text-blue-800">
