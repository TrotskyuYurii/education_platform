...
export const CourseCatalog: React.FC<CourseCatalogProps> = ({
  ...
  const filteredItems = useMemo(() => {
    ...
    const cItems = courses.filter(c => {
      ...
      const matchesSpace = selectedSpaceId === 'all' || c.spaceId === selectedSpaceId ||
        sections.some(s => c.instructionIds?.includes(s.id) && s.spaceId === selectedSpaceId);
      const searchLower = searchQuery.toLowerCase().trim();
      const matchesSearch = searchLower === '' || 
        (c.title || '').toLowerCase().includes(searchLower) ||
        (c.department || '').toLowerCase().includes(searchLower);
      return matchesDept && matchesSpace && matchesSearch;
    }).map(c => {
    ...
    const sItems = sections.filter(sec => {
      ...
      const matchesSpace = selectedSpaceId === 'all' || (sec.spaceId || 'space-general') === selectedSpaceId;
      const searchLower = searchQuery.toLowerCase().trim();
      const matchesSearch = searchLower === '' || 
        ...
        <div className="flex items-start gap-4 flex-1">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              <span className="inline-flex items-center gap-0.5 text-[10px] font-mono font-bold text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded-md">
                v{item.version || '1.0'}
              </span>
              {item.status && item.status !== 'published' && (
                <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded-md ${
                  item.status === 'draft' ? 'bg-amber-100 text-amber-800' :
                  item.status === 'in_review' ? 'bg-indigo-100 text-indigo-800' :
                  'bg-slate-200 text-slate-700'
                }`}>
                  {item.status === 'draft' ? 'Чернетка' : item.status === 'in_review' ? 'Рецензія' : 'Архів'}
                </span>
              )}
            </div>
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-1 max-h-48 overflow-y-auto pr-1">
                    <button
                      ...
                      className={`flex items-center justify-between px-3 py-2 rounded-xl text-xs font-semibold transition text-left ${
                        selectedSpaceId === 'all'
                    ...
                    >
                      <div className="flex items-center gap-2 truncate">
                        ...
                        <span className="truncate">Всі простори</span>
                      </div>
                    </button>

                    {spaces.map(sp => {
                      const countInSpace = sections.filter(s => (s.spaceId || 'space-general') === sp.id).length;
                      return (
                        <button
                          key={sp.id}
                        ...
                        >
                          ...
                          <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${
                            selectedSpaceId === sp.id ? 'bg-blue-700 text-white' : 'bg-slate-200 text-slate-600'
                          }`}>
                            {countInSpace}
                          </span>
                        </button>
                ...
                <div>
                  ...
                  >
                    ...
                    {departments.map((dep, idx) => {
                      ...
                      return <option key={`cat-dept-${label}-${idx}`} value={label}>{label}</option>;
                    })}
                  </select>
                </div>
              )}
